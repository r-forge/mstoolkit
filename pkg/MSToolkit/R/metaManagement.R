...
assign( "verbose", FALSE, env = .ectdEnv )
...
assign("dataStoreMethod", "CSV", env = .ectdEnv)
...
setEctdDataMethod <- function(method) {
	...
	assign("dataStoreMethod", method, env = .ectdEnv )
	invisible(method)
}

# Get & Set external execution path
getEctdExternalPath <- function( pathName ) {
	getPaths <- get("externalPaths", env = .ectdEnv)
	if (missing(pathName)) {
...
setEctdExternalPath <- function(pathName, Value) {
	...
	assign("externalPaths", getPaths, env = .ectdEnv)
	invisible(getPaths)
}
...
getEctdColName <- function(colName) {
	...
	getNames <- get("colNames", env = .ectdEnv)
...
setEctdColName <- function(colName, Value) {
	...
	assign("colNames", getNames, env = .ectdEnv)
	invisible(getNames)
}
