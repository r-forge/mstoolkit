createMCAR <- function(
  ...
  if( flagName %in% colnames(data) && data[[flagName]] %!allin% c(0,1)  ){
  ...
  if( missing(rule)){ 
    ...
    if( ! any( include ) ){
      ectdStop( "All logical values are false"  )
    }    
  } 
  
  ### sample the missing values
  missingSample <- sample( c(1, 0), 
    prob = c( prop, 1 - prop), 
    size = sum(include), replace = TRUE )
